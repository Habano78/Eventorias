...
struct ProfileView: View {
    ...
    var myCreatedEvents: [Event] {
        let officialID = currentUser?.fireBaseUserId ?? Auth.auth().currentUser?.uid
        
        guard let userID = officialID else { return [] }
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear {
                ...
                // Sécurité : Si le ViewModel est vide, on relance un fetch discret
                if authViewModel.currentUser == nil, let userID = Auth.auth().currentUser?.uid {
